...
class UFSC_Menu
{
    ...
    public function render_liste_licences_page()
    {
        ...
        // Handle CSV export
        if (isset($_GET['export_csv']) && check_admin_referer('ufsc_export_licences_all')) {
            $this->process_main_license_export();
            return;
        }
        ...
        // Enqueue DataTables for enhanced functionality
        wp_enqueue_style(
            'datatables-css',
        ...
        <div class="wrap">
            ...
            <p><?php echo esc_html__('Gérez toutes les licences enregistrées dans le système. Utilisez les filtres pour rechercher des licences spécifiques.', 'plugin-ufsc-gestion-club-13072025'); ?></p>
            
            <!-- Add new licence button -->
            <p>
            ...
            <?php 
            UFSC_Licence_Filters::render_filters_form($filters, 'ufsc-liste-licences', 0, true);
            UFSC_Licence_Filters::render_filters_css();
            ?>
            
            <!-- Results summary -->
            <div class="ufsc-results-summary">
                <p><strong><?php echo esc_html($total_items); ?></strong> licence(s) trouvée(s)
                <?php if ($total_items != $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->prefix}ufsc_licences")): ?>
                    sur <?php echo esc_html($wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->prefix}ufsc_licences")); ?> au total
                <?php endif; ?>
                </p>
            </div>
            
            <div class="ufsc-licences-table-wrapper">
                <!-- Enhanced table with DataTables -->
                <table id="licenses-table-global" class="widefat fixed striped display nowrap" style="width:100%">
                    <thead>
                        <tr>
                            <th><?php echo esc_html__('ID', 'plugin-ufsc-gestion-club-13072025'); ?></th>
                            <th><?php echo esc_html__('Nom', 'plugin-ufsc-gestion-club-13072025'); ?></th>
                            <th><?php echo esc_html__('Prénom', 'plugin-ufsc-gestion-club-13072025'); ?></th>
                            <th><?php echo esc_html__('Sexe', 'plugin-ufsc-gestion-club-13072025'); ?></th>
                            <th><?php echo esc_html__('Date de naissance', 'plugin-ufsc-gestion-club-13072025'); ?></th>
                            <th><?php echo esc_html__('Email', 'plugin-ufsc-gestion-club-13072025'); ?></th>
                            <th><?php echo esc_html__('Ville', 'plugin-ufsc-gestion-club-13072025'); ?></th>
                            <th><?php echo esc_html__('Région', 'plugin-ufsc-gestion-club-13072025'); ?></th>
                            <th><?php echo esc_html__('Club', 'plugin-ufsc-gestion-club-13072025'); ?></th>
                            <th><?php echo esc_html__('Compétition', 'plugin-ufsc-gestion-club-13072025'); ?></th>
                            <th><?php echo esc_html__('Inclus', 'plugin-ufsc-gestion-club-13072025'); ?></th>
                            <th><?php echo esc_html__('Date d\'inscription', 'plugin-ufsc-gestion-club-13072025'); ?></th>
                            <th><?php echo esc_html__('Actions', 'plugin-ufsc-gestion-club-13072025'); ?></th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php if ($licences): ?>
                            <?php foreach ($licences as $licence): ?>
                                <tr>
                                    <td><?php echo esc_html($licence->id); ?></td>
                                    <td><?php echo esc_html($licence->nom); ?></td>
                                    <td><?php echo esc_html($licence->prenom); ?></td>
                                    <td>
                                        <span class="ufsc-badge <?php echo $licence->sexe === 'F' ? 'badge-pink' : 'badge-blue'; ?>">
                                            <?php echo $licence->sexe === 'F' ? 'Femme' : 'Homme'; ?>
                                        </span>
                                    </td>
                                    <td><?php echo esc_html($licence->date_naissance); ?></td>
                                    <td><?php echo esc_html($licence->email); ?></td>
                                    <td><?php echo esc_html($licence->ville); ?></td>
                                    <td><?php echo esc_html($licence->region); ?></td>
                                    <td><strong><?php echo esc_html($licence->club_nom); ?></strong></td>
                                    <td>
                                    ...
                                    <td>
                                        <span class="ufsc-badge <?php echo $licence->is_included ? 'badge-green' : 'badge-red'; ?>">
                                            <?php echo $licence->is_included ? 'Inclus' : 'Payant'; ?>
                                        </span>
                                    </td>
                                    <td><?php echo esc_html(date('d/m/Y', strtotime($licence->date_inscription))); ?></td>
                                    <td>
                                        <div class="ufsc-actions">
<button type="button" class="button button-small change-status-approve" data-new-status="validee" data-licence-id="<?php echo esc_attr($licence->id); ?>">✅ Valider</button>
<button type="button" class="button button-small change-status-refuse" data-new-status="refusee" data-licence-id="<?php echo esc_attr($licence->id); ?>">⛔ Refuser</button>

                                            <a href="<?php echo esc_url(admin_url('admin.php?page=ufsc_view_licence&id=' . $licence->id)); ?>" 
                                               class="button button-small" title="Voir les détails">
                                                👁️ Voir
                                            </a>
                                            <a href="<?php echo esc_url(admin_url('admin.php?page=ufsc-modifier-licence&licence_id=' . $licence->id)); ?>" 
                                            ...
                                            <button type="button" 
                                                    ...
                                                    data-licence-id="<?php echo esc_attr($licence->id); ?>"
                                                    data-licence-name="<?php echo esc_attr($licence->prenom . ' ' . $licence->nom); ?>"
                        ...
                        <?php else: ?>
                            <tr>
                                <td colspan="13"><?php echo esc_html__('Aucune licence trouvée avec les critères sélectionnés.', 'plugin-ufsc-gestion-club-13072025'); ?></td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            ...
            <!-- Pagination (handled by DataTables now, but keeping for server-side pagination if needed) -->
