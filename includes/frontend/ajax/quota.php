...
function ufsc_ajax_include_quota(){
    if (!ufsc_check_ajax_nonce('ufsc_front_nonce', '_ajax_nonce', false)) wp_send_json_error(esc_html__('Bad nonce', 'ufsc-domain'),403);
    if (!is_user_logged_in()) wp_send_json_error(esc_html__('Non connecté', 'ufsc-domain'),401);
    ...
    if (!$licence_id || !$club_id) wp_send_json_error(esc_html__('Données manquantes', 'ufsc-domain'));
    ...
    if ($allowed && $used >= $allowed) wp_send_json_error(esc_html__('Quota atteint', 'ufsc-domain'));
