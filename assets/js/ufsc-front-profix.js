...
function post(a,d){d=d||{};d.action=a;if(nonce&&!d.ufsc_nonce)d.ufsc_nonce=nonce;return $.post(ajaxUrl,d);}
$(document).on('click','.ufsc-pay-licence,.ufsc-add-to-cart',function(e){e.preventDefault();var $b=$(this),id=$b.data('licenceId')||$b.data('licence-id');if(!id)return;$b.prop('disabled',true);post('ufsc_add_to_cart',{licence_id:id,ufsc_nonce:$b.data('nonce')}).done(function(r){if(r&&r.success&&r.data&&r.data.redirect){window.location.href=r.data.redirect;}else{toast((r&&r.data&&r.data.message)||'Erreur');}}).fail(function(){toast('Erreur réseau');}).always(function(){$b.prop('disabled',false);});});
